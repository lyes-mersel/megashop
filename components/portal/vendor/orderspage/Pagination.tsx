...
export default function Pagination({
  ...
  useEffect(() => {
    ...
    const calculateVisiblePages = () => {
      ...
      if (totalPages <= 7) {
        ...
        if (currentPage <= 3) {
          // If we're near the beginning, show first 5 pages, then ellipsis, then last page
          newVisiblePages.push(2, 3, 4, 5, "...", totalPages);
        } else if (currentPage >= totalPages - 2) {
          // If we're near the end, show first page, ellipsis, then last 5 pages
          ...
          // We're in the middle, show first page, ellipsis, current -1, current, current + 1, ellipsis, last page
          newVisiblePages.push(
            "...",
            currentPage - 1,
            currentPage,
  ...
  // Calculate range of items showing (e.g. "Showing 1-10 of 50 items")
  ...
  return (
    <div className="mt-6 mb-8 flex flex-col sm:flex-row justify-between items-center gap-4">
      <p className="text-sm text-gray-600">
        ...
        <span className="font-medium">{totalItems}</span> commandes
      </p>
